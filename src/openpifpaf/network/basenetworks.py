...
class ResnetDC5(BaseNetwork):
    pretrained = True

    def __init__(self, name, torchvision_resnet, dilation):
        stride = 32
        if dilation:
            stride //= 2
        num_channels = 512 if name in ('resnet18', 'resnet34') else 2048
        super().__init__(name, stride=stride, out_features=num_channels)
        backbone = torchvision_resnet(
            replace_stride_with_dilation=[False, False, dilation],
            pretrained=self.pretrained, norm_layer=FrozenBatchNorm2d)

        self.model = IntermediateLayerGetter(backbone, return_layers={'layer4': "0"})
