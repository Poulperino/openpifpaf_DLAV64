        ...
        for target, headname, stride in zip(targets, self.head_names, self.strides):
            ...
            if 'butterfly' in headname:
                ...
                if 'nsbutterfly' in headname:
                    scale_wh = 1
                self.butterfly(image, target, meta, stride, scale_wh, keypoint_sets, keypoints=BBOX_KEYPOINTS)
            elif 'repulse' in headname:
                scale_wh = stride
                if 'nsrepulse' in headname:
                    scale_wh = 1
                self.butterfly_repulse(image, target, stride, scale_wh, keypoint_sets, keypoints=BBOX_KEYPOINTS)
            else:
